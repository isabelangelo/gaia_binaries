...
flux_source_id_list = [int(i) for i in pw2020_flux_data.columns]
pw2020_binaries = pw2020_binaries[pw2020_binaries['source_id'].isin(flux_source_id_list)]
...
def get_labels(label_df, flux_data_df, flux_err_data_df, inst='GALAH'):
    ...
    original_dataset_keys = keys[-7:]
    data = []

    # iterate over each object
    for source_id in label_df.source_id.to_numpy()[:5]:

        # store galah labels
        row = label_df.loc[label_df.source_id==source_id]
        original_dataset_labels = row[keys[:5]].to_numpy()
