...
galah_dopplegangers_gaia = pd.merge(galah_gaia_xmatch[galah_gaia_xmatch_cols], 
	galah_allstar_catalog[galah_allstar_catalog_cols], on='sobject_id')
galah_dopplegangers_gaia = galah_dopplegangers_gaia.drop_duplicates(subset='designation', keep='first')
galah_dopplegangers_gaia = galah_dopplegangers_gaia[galah_dopplegangers_gaia.designation!=' ']
print(len(galah_dopplegangers_gaia), 'stars with unique gaia designations from GALAH/gaia crossmatch')

# save relevant parameters, write to file
galah_dopplegangers_gaia = galah_dopplegangers_gaia.rename(
    columns={
    "teff": "galah_teff", 
    "e_teff": "galah_eteff",
    "logg":"galah_logg",
...
galah_dopplegangers_gaia = galah_dopplegangers_gaia.query('galah_logg > 4')
print(len(galah_dopplegangers_gaia), 'with logg>4')
...
for i in range(len(galah_dopplegangers_gaia)):
    row = galah_dopplegangers_gaia.iloc[i]
    if row.sobject_id in binary_galah_ids:
        binary_idx_to_remove.append(i)
galah_dopplegangers_gaia = galah_dopplegangers_gaia.drop(galah_dopplegangers_gaia.index[binary_idx_to_remove])
print(len(galah_dopplegangers_gaia), 'remaining after removing binaries from Traven et al. 2020')

# index random set of 5000
# (just for now since it takes so long to upload to gaia)
galah_dopplegangers_gaia = galah_dopplegangers_gaia.sample(5000)
...
dr3.ra, dr3.dec \
FROM user_iangelo.galah_dopplegangers_gaia as galah \
...
gaia.upload_table(galah_dopplegangers_gaia, 'galah_dopplegangers_gaia')
galah_dopplegangers_gaia_results, galah_dopplegangers_flux_df, galah_dopplegangers_sigma_df = gaia.retrieve_data_and_labels(query)
print('{} with has_rvs = True, snr cuts, non_single_star = 0'.format(len(galah_dopplegangers_gaia_results)))

print('saving flux, flux_err to .csv files')
gaia.write_labels_to_file(galah_dopplegangers_gaia_results, 'galah_binaries')
gaia.write_flux_data_to_csv(galah_dopplegangers_flux_df, galah_dopplegangers_sigma_df, 'galah_binaries')
