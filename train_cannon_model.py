...
# model_0.write(model_filename_initial, include_training_set_spectra=True, overwrite=True)
...
# print('iteration {}: {} binaries found, next iteration training set size = {}'.format(
#     n_iter, 
#     n_binaries, 
...
# model_cleaned.write(model_filename_cleaned, include_training_set_spectra=True, overwrite=True)
...
# note: this is not using the recent model version from custom_model, since
# that model gets updated after this script is run
print('generating one-to-one diagnostic plot with leave-one-out cross-validation')
# define path to save plots to
cleaned_model_fileroot = model_fileroot + '_cleaned'
model_figure_path = './data/cannon_models/'+cleaned_model_fileroot+'_figures/'
...
# note: I am loading the model from file to validate the model that was written,
# since this is the one that is used in the paper analysis
model_cleaned_from_file = tc.CannonModel.read('./data/cannon_models/'+cleaned_model_fileroot+'.model')
plot_one_to_one(
    model_cleaned_from_file,
    training_label_df_cleaned,
    training_flux_df_cleaned,
    training_sigma_df_cleaned,
    model_figure_path + 'one_to_one.png',
    path_to_save_labels = cleaned_model_fileroot+'_training_labels')
